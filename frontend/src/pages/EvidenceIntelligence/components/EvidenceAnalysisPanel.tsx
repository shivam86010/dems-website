...
// import { Cpu, Shield, Copy, CheckCheck, Video, Download, ExternalLink, Scan, Car, Package } from 'lucide-react';


// // Backend integration point: replace with API call to /api/evidence/:id/analysis
// const analysisData = {
//   id: 'evd-4441',
//   label: 'EVD-4441',
...
//             <span className="font-mono text-sm font-bold text-primary">{analysisData?.label}</span>
//             <span className="text-2xs font-semibold px-2 py-0.5 rounded-full bg-cyan-500/15 text-cyan-400 border border-cyan-500/20 uppercase">
//               Blockchain Sealed
//             </span>
//           </div>
//           <p className="text-xs text-foreground font-medium">{analysisData?.filename}</p>
//           <p className="text-2xs text-muted-foreground mt-0.5">
//             {analysisData?.camera} · {analysisData?.duration}
//           </p>
//         </div>
//         <div className="flex gap-2 shrink-0">
//           <button className="btn-ghost text-xs gap-1.5 h-8">
//             <Download size={13} />
//             Export
//           </button>
//           <button className="btn-ghost text-xs gap-1.5 h-8">
...
//           <p className="text-2xs font-semibold text-muted-foreground uppercase tracking-wider mb-0.5">SHA-256 Hash</p>
//           <p className="hash-text truncate">{analysisData?.hash}</p>
//         </div>
//         <button
//           onClick={copyHash}
...
//         <span className="text-2xs text-muted-foreground">Analyzed by</span>
//         <span className="text-2xs font-semibold text-violet-400">{analysisData?.aiModel}</span>
//         <span className="text-2xs text-muted-foreground ml-auto">{analysisData?.aiProcessedAt}</span>
//       </div>
//       {/* Tabs */}
...
//               <Scan size={13} className="text-violet-400" />
//               <span className="text-xs font-semibold text-foreground">Face Detections ({analysisData?.faces?.length})</span>
//             </div>
//             <div className="space-y-2">
//               {analysisData?.faces?.map((face) => (
//                 <div key={face?.id} className={`p-3 rounded-lg border ${face?.status === 'matched' ? 'border-red-500/30 bg-red-500/5' : 'border-border bg-muted/20'}`}>
//                   <div className="flex items-center justify-between mb-1">
//                     <span className="text-xs font-semibold text-foreground">{face?.label}</span>
//                     <div className="flex items-center gap-2">
//                       <span className="font-mono text-2xs text-muted-foreground">@ {face?.timestamp}</span>
//                       <span className={`text-2xs font-bold ${face?.confidence >= 90 ? 'text-success' : face?.confidence >= 75 ? 'text-amber-400' : 'text-muted-foreground'}`}>
//                         {face?.confidence}%
//                       </span>
//                     </div>
//                   </div>
//                   <p className={`text-2xs ${face?.status === 'matched' ? 'text-red-400 font-semibold' : 'text-muted-foreground'}`}>
//                     {face?.match}
//                   </p>
//                 </div>
//               ))}
...
//               <span className="text-xs font-semibold text-foreground">License Plates ({analysisData?.plates?.length})</span>
//             </div>
//             {analysisData?.plates?.map((plate) => (
//               <div key={plate?.id} className="p-3 rounded-lg border border-amber-500/30 bg-amber-500/5">
//                 <div className="flex items-center justify-between mb-1">
//                   <span className="font-mono text-sm font-bold text-amber-400">{plate?.value}</span>
//                   <div className="flex items-center gap-2">
//                     <span className="font-mono text-2xs text-muted-foreground">@ {plate?.timestamp}</span>
//                     <span className="text-2xs font-bold text-amber-400">{plate?.confidence}%</span>
//                   </div>
//                 </div>
//                 <p className="text-2xs text-muted-foreground">{plate?.make}</p>
//                 <span className="text-2xs font-bold text-red-400 mt-1 block">⚠ WANTED VEHICLE</span>
//               </div>
//             ))}
//           </div>
...
//               <span className="text-xs font-semibold text-foreground">Object Detections ({analysisData?.objects?.length})</span>
//             </div>
//             <div className="space-y-1.5">
//               {analysisData?.objects?.map((obj) => (
//                 <div key={obj?.id} className="flex items-center gap-3 p-2.5 rounded-lg border border-border bg-muted/10">
//                   <span className={`text-xs font-semibold ${obj?.label === 'Knife' ? 'text-red-400' : 'text-foreground'}`}>
//                     {obj?.label === 'Knife' && '⚠ '}{obj?.label}
//                   </span>
//                   <span className="font-mono text-2xs text-muted-foreground ml-auto">@ {obj?.timestamp}</span>
//                   <span className="text-2xs font-bold text-blue-400">{obj?.confidence}%</span>
//                   <span className="hash-text">{obj?.bbox}</span>
//                 </div>
//               ))}
//             </div>
//           </div>
...
//             <span className="text-xs font-semibold text-foreground">AI-Generated Case Summary</span>
//             <span className="text-2xs text-muted-foreground ml-auto">Generated by LLM v2.1</span>
//           </div>
...
//             { label: 'Duration', value: analysisData?.duration },
//             { label: 'Camera Source', value: analysisData?.camera },
//             { label: 'Uploaded By', value: analysisData?.uploadedBy },
//             { label: 'Upload Time', value: analysisData?.uploadedAt },
//             { label: 'AI Processed', value: analysisData?.aiProcessedAt },
//             { label: 'Blockchain TX', value: analysisData?.blockchainTxId },
//             { label: 'Chain Sync', value: 'Verified · 14 May 2026, 07:20 IST' },
//           ]?.map((row) => (
//             <div key={`meta-${row?.label}`} className="flex items-center justify-between py-2 border-b border-border/50">
//               <span className="text-2xs font-semibold text-muted-foreground uppercase tracking-wider">{row?.label}</span>
//               <span className="hash-text text-right">{row?.value}</span>
//             </div>
//           ))}
//         </div>
//       )}
...
// components/EvidenceAnalysisPanel.tsx
import { useState } from 'react';
import { Cpu, Shield, Copy, CheckCheck, Video, Download, ExternalLink, Scan, Car, Package, Sparkles, Brain, Fingerprint, Calendar, User, Camera, Clock, Hash } from 'lucide-react';
