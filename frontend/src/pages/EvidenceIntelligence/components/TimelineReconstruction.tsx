...
// import { Clock, MapPin, Video, Mic, FileText, User, Shield } from 'lucide-react';
...
//   const [expanded, setExpanded] = useState<string | null>('evt-003');
...
//           </p>
//         </div>
//         <span className="text-2xs font-semibold text-amber-400 px-2 py-1 rounded-full bg-amber-500/10 border border-amber-500/20">
...
//                 {/* Icon node */}
//                 <div className={`absolute left-0 w-[55px] flex justify-center`} style={{ top: '10px' }}>
//                   <div className={`w-9 h-9 rounded-full ${evt?.bg} border border-border flex items-center justify-center z-10 relative`}>
...
//                     isExpanded ? 'border-border/80 bg-muted/20' : 'border-transparent hover:border-border/40 hover:bg-muted/10'
//                   }`}
//                 >
//                   <div className="flex items-center justify-between mb-1">
//                     <span className="font-mono text-xs font-bold text-foreground">{evt?.time}</span>
//                     <span className={`text-2xs font-bold ${evt?.confidence >= 90 ? 'text-success' : evt?.confidence >= 75 ? 'text-amber-400' : 'text-orange-400'}`}>
//                       {evt?.confidence}% conf.
//                     </span>
//                   </div>
//                   <p className={`text-xs font-semibold ${evt?.type === 'object' ? 'text-red-400' : 'text-foreground'}`}>
//                     {evt?.title}
//                   </p>
...
//                         <span className="text-2xs text-muted-foreground">{evt?.location}</span>
//                       </div>
//                       <div className="flex items-center gap-2">
//                         <FileText size={11} className="text-primary shrink-0" />
//                         <span className="hash-text">{evt?.source}</span>
//                       </div>
//                     </div>
//                   )}
//                 </button>
...
// components/TimelineReconstruction.tsx
import React, { useState } from 'react';
import { Clock, MapPin, Video, Mic, FileText, User, Shield, Calendar, Target, AlertTriangle, Navigation } from 'lucide-react';
